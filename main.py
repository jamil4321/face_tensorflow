...
broker = "127.0.0.1"
port = 1883
url = 'http://127.0.0.1:2000/send_notifications'
cam = "http://192.168.0.100/capture"
...
def on_publish(client, userdata):  # create function for callback
    print("data published \n")
    pass


def on_message(client, userdata, msg):
    global mtcnn, images_placeholder, phase_train_placeholder, tf_sess, embeddings, id_data
    if msg.payload.decode() == "bell":
        conunt = 0
        flag = True
        print('abc')
        ...
        while flag:
            ...
            if len(face_patches) > 0:
                ...
                embs = tf_sess.run(embeddings, feed_dict=feed_dict)

                matching_ids, matching_distances = id_data.find_matching_ids(
                    embs)
                frame_detections = {
                    "embs": embs, "bbs": padded_bounding_boxes, "frame": frame.copy()}
                ...
                conunt += 1
                print("Couldn't find a face", conunt)
                if conunt == 20:
                    return
    elif msg.payload.decode() == "train":
...
def main(args):
    with tf.Graph().as_default():
        with tf.Session() as sess:
            global mtcnn, images_placeholder, phase_train_placeholder, embeddings, id_data, tf_sess, new_args
            tf_sess = sess
            new_args = args
            # Setup models
            ...
            client1 = paho.Client("SmartEyeCamera")
            client1.connect(broker, port)
            client1.on_publish = on_publish
            client1.subscribe("O1001/Places/Indoor/GroundFloor/mainGate/Bell")
